...
def _restore_graph_cache(runner, model_name, memory_saver_adapter=None):
    ...
    try:
        ...
        if memory_saver_adapter is not None and memory_saver_adapter.enabled and vmm_tag:
...
def _scatter_gather_weights(si, blocks, bump, weights_start, cpu_sd, total_weight_target, layer_map=None):
    """D2D gather valid chunks from staging, H2D corrupted chunks from CPU.

    T3 optimization: consecutive valid/invalid chunks are merged into runs.
    Each run is a single copy per block instead of per-chunk copies.
    E.g., [T,T,F,T,T,T] → 3 runs → 120×2 D2D + 120×1 H2D instead of 2040 copies.
    ...
    while i < num_chunks:
        ...
        while j < num_chunks and chunk_valid[j] == chunk_valid[i]:
            j += 1
        runs.append((chunk_valid[i], i, j))  # (is_valid, start_chunk, end_chunk_exclusive)
        i = j
    ...
    for is_valid, c_start, c_end in runs:
        # Byte range for this run of chunks
        byte_off_start = c_start * chunk_rows * si.row_size
        # Row range: staging_start + c_start*chunk_rows .. min(staging_start + c_end*chunk_rows, pool_size)
        run_row_start = si.staging_start + c_start * chunk_rows
        run_row_end = min(si.staging_start + c_end * chunk_rows,
        ...
        if is_valid:
            # Skip unsafe blocks: staging data starts from blocks[unsafe_blocks]
            src_off = run_row_start * si.row_size
            with torch.cuda.stream(d2d_stream):
                for bi in range(si.num_blocks):
                    src = blocks[_ub + bi].view(-1).view(torch.uint8)[src_off : src_off + nbytes]
            ...
            with torch.cuda.stream(h2d_stream):
                for bi in range(si.num_blocks):
                    cpu_off = bi * si.staged_per_block + byte_off_start
    ...
    if runs:
        logger.debug(
            f"  D2D runs: {len(runs)} runs from {num_chunks} chunks "
            f"(copies: {len(runs)*si.num_blocks} vs old {num_chunks*si.num_blocks})"
        )

    # H2D for unstaged params (didn't fit in staging)
    # Note: unsafe blocks don't need separate H2D because staging data starts
    # from param byte 0 (staged in safe blocks, D2D'd to bump offset 0).
    _total_covered = si.total_staged_bytes
    if cpu_sd is not None and _total_covered < total_weight_target:
        with torch.cuda.stream(h2d_stream):
            _h2d_remaining_params(
                cpu_sd, bump,
                weights_start,
                _total_covered, total_weight_target,
                layer_map=layer_map,
            )
...
def _h2d_remaining_params(cpu_sd: dict, bump, weights_start: int,
                          skip_bytes: int, total_bytes: int, layer_map=None):
    """H2D params that weren't staged, using layer_map for correct offsets.

    If layer_map is provided, each param is placed at its exact bump offset
    (from create_tensor). Only params with offset >= skip_bytes are copied.
    """
    if layer_map:
        for ls in layer_map:
            rel_offset = ls.offset - weights_start
            if rel_offset + ls.nbytes <= skip_bytes:
                continue
            if rel_offset >= total_bytes:
                continue
            tensor = cpu_sd.get(ls.name)
            if tensor is None:
                continue
            raw_bytes = min(ls.nbytes, tensor.numel() * tensor.element_size())
            # Handle partial overlap with skip_bytes
            start_in_param = max(0, skip_bytes - rel_offset)
            take = min(raw_bytes - start_in_param, total_bytes - max(rel_offset, skip_bytes))
            if take <= 0:
                continue
            cpu_bytes = tensor.contiguous().view(-1).view(torch.uint8)[start_in_param : start_in_param + take]
            write_off = weights_start + max(rel_offset, skip_bytes)
        ...
        for name, tensor in cpu_sd.items():
            ...
            end = pos + aligned_bytes
            if end <= skip_bytes:
                pos = end
                continue
            start_in_param = max(0, skip_bytes - pos)
            raw_remaining = raw_bytes - start_in_param
            space_left = total_bytes - max(pos, skip_bytes)
            take = min(raw_remaining, space_left)
            if take <= 0:
                break
            cpu_bytes = tensor.contiguous().view(-1).view(torch.uint8)[start_in_param : start_in_param + take]
...
def do_model_switch_bump(scheduler, target_model_path, target_model_name=None):
    ...
    logger.info(f"  save_teardown: {timings['save_teardown']*1000:.1f}ms")
    ...
    if _pt is not None and _pt.is_alive():
        ...
        _pt.join()
        logger.info(f"  preload_join: {(time.perf_counter()-_t_join)*1000:.1f}ms")
    ...
    logger.info(f"  config_update: {(_t_config_end-_t_config)*1000:.1f}ms")
    ...
    logger.info(f"  get_cpu_model: {(time.perf_counter()-_t_cpu)*1000:.1f}ms")
    ...
    if _cached_model is not None and _has_staging:
        ...
        logger.info(f"  layer_map: {(time.perf_counter()-_t_lmap)*1000:.1f}ms")
        if needed_bytes > 0 and needed_bytes != current_cap:
            _t_rel = time.perf_counter()
            bump.release_region("kv_cache")
            logger.info(f"  kv_release: {(time.perf_counter()-_t_rel)*1000:.1f}ms")
            _t_resize = time.perf_counter()
            bump.reset_region("weights", needed_bytes)
            _t_resize_end = time.perf_counter()
            logger.info(f"  Weights region resized: {current_cap/1024**2:.1f}MB -> {needed_bytes/1024**2:.1f}MB time={(_t_resize_end-_t_resize)*1000:.1f}ms")
            logger.debug(f"  DIAG post-resize: weights=[{bump.regions['weights'].start}, {bump.regions['weights'].start + needed_bytes}) bump.left_offset={bump.left_offset} old_cap={current_cap} new_cap={needed_bytes}")
            weights_start = bump.regions["weights"].start

        _t_h2d_src = time.perf_counter()
        h2d_src = _cpu_sd
        if h2d_src is None and _cpu_model is not None:
            h2d_src = {k: v.detach().cpu().clone() for k, v in _cpu_model.state_dict().items()}
        logger.info(f"  h2d_src_prep: {(time.perf_counter()-_t_h2d_src)*1000:.1f}ms")

        total_target = needed_bytes or current_cap
        _t_diag_pre = time.perf_counter()
        # === DIAG: comprehensive pre-D2D logging ===
        _ws = weights_start
        _bump_addr = bump.buffer.data_ptr()
        _si_diag = _preload_mgr.scatter_info
        logger.debug(f"  DIAG weights_region: start={_ws} cap={bump.regions['weights'].capacity} bump.left_offset={bump.left_offset}")
        logger.debug(f"  DIAG total_target={total_target} needed_bytes={needed_bytes} current_cap={current_cap}")
        logger.debug(f"  DIAG staging_info: staging_start={_si_diag.staging_start} staging_rows={_si_diag.staging_rows} staged_per_block={_si_diag.staged_per_block} row_size={_si_diag.row_size} pool_size={_si_diag.pool_size} num_blocks={_si_diag.num_blocks} total_staged_bytes={_si_diag.total_staged_bytes}")
        _weights_region_end = _ws + total_target
        _kv_pool_phys_start = bump.left_offset
        logger.debug(f"  DIAG addresses: bump_base=0x{_bump_addr:x} weights_region=[{_ws}, {_weights_region_end}) kv_pool_phys_start={_kv_pool_phys_start}")
        for _bi in (range(min(5, len(_preload_mgr.block_tensors))) if _SWITCH_DIAG else []):
            _blk = _preload_mgr.block_tensors[_bi]
            _blk_start = _blk.data_ptr() - _bump_addr
            _blk_end = _blk_start + _blk.numel() * _blk.element_size()
            _staging_off = _blk_start + _si_diag.staging_start * _si_diag.row_size
            _staging_end = _blk_start + _si_diag.pool_size * _si_diag.row_size
            _overlap = _staging_off < _weights_region_end
            logger.info(f"  DIAG block[{_bi}]: phys=[{_blk_start}, {_blk_end}) staging_phys=[{_staging_off}, {_staging_end}) overlap_with_weights={_overlap} gap={_staging_off - _weights_region_end}")
        logger.info(f"  diag_pre_logging: {(time.perf_counter()-_t_diag_pre)*1000:.1f}ms")
        _t_d2d = time.perf_counter()
        ...
        _t_d2d_end = time.perf_counter()
        logger.info(f"  D2D scatter-gather: d2d={d2d_bytes/1024**2:.1f}MB h2d={h2d_bytes/1024**2:.1f}MB time={(_t_d2d_end-_t_d2d)*1000:.1f}ms")
        if _SWITCH_DIAG:
                # DIAG: verify D2D gather - compare staging source vs bump dest
            torch.cuda.synchronize()
            _diag_si = _preload_mgr.scatter_info
            _diag_blocks = _preload_mgr.block_tensors
            _diag_ok = 0
            _diag_bad = 0
            for _bi in range(min(3, _diag_si.num_blocks)):
                _blk_flat = _diag_blocks[_bi].view(-1).view(torch.uint8)
                for _ci in range(_diag_si.chunk_rows and len(_diag_si.chunk_valid or [])):
                    if not (_diag_si.chunk_valid or [])[_ci]:
                        continue
                    _row_s = _diag_si.staging_start + _ci * _diag_si.chunk_rows
                    _row_e = min(_row_s + _diag_si.chunk_rows, _diag_si.pool_size)
                    _byte_off = _ci * _diag_si.chunk_rows * _diag_si.row_size
                    _nb = min((_row_e - _row_s) * _diag_si.row_size, _diag_si.staged_per_block - _byte_off)
                    if _nb <= 0:
                        break
                    _src_off = _row_s * _diag_si.row_size
                    _staging_bytes = _blk_flat[_src_off:_src_off + _nb]
                    _dst_off = weights_start + _bi * _diag_si.staged_per_block + _byte_off
                    _bump_bytes = bump.buffer[_dst_off:_dst_off + _nb]
                    if torch.equal(_staging_bytes, _bump_bytes):
                        _diag_ok += 1
                    else:
                        _ndiff = (_staging_bytes != _bump_bytes).sum().item()
                        logger.error(f"  D2D CHUNK MISMATCH: block={_bi} chunk={_ci} {_ndiff}/{_nb} bytes differ")
                        _diag_bad += 1
            logger.info(f"  D2D chunk verify: {_diag_ok} OK, {_diag_bad} BAD (checked blocks 0-2)")
        ...
        logger.info(f"  model_assign: {(_t_eval-_t_assign)*1000:.1f}ms  eval: {(_t_eval_end-_t_eval)*1000:.1f}ms")
        ...
        for name, buf in runner.model.named_buffers():
            if buf is not None and buf.device.type == "cpu":
                ...
                _buf_count += 1
                _buf_bytes += buf.numel() * buf.element_size()
        logger.info(f"  buf_migration: {(time.perf_counter()-_t_buf)*1000:.1f}ms ({_buf_count} bufs, {_buf_bytes/1024:.1f}KB)")
        ...
        _t_rope_end = time.perf_counter()
        logger.info(f"  RoPE+buffers: {(_t_rope_end-_t_rope)*1000:.1f}ms")
    ...
    if _kv_hit:
        ...
        # Evict stale KV pool cache entry (weight_bytes mismatch or first load)
        _kv_pool_cache.pop(target_model_name, None)
        if "kv_cache" in bump.regions:
            bump.release_region("kv_cache")
        # Force flush: tree_cache.reset() + allocator.clear() (bypass idle check)
        scheduler.tree_cache.reset()
    ...
    logger.info(f"  kv_cache: {timings['kv_cache']*1000:.1f}ms")

    # Update served_model_name BEFORE graph recapture so VMM tags are correct
    scheduler.server_args.served_model_name = target_model_name
