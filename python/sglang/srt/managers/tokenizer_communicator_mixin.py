...
class TokenizerCommunicatorMixin:
    ...
    async def register_model(
        ...
        self.auto_create_handle_loop()
        # 1. Kick off background local tokenizer load + mark pending.
        ...
        result = await self.register_model_communicator(obj)
        # 3. Rollback local registration if scheduler rejected (e.g. conflict).
        if not getattr(result, "success", True):
            self.model_tokenizers.pop(obj.model_name, None)
            self._model_readiness.pop(obj.model_name, None)
            logger.warning(
                f"register_model rollback: scheduler rejected '{obj.model_name}' "
                f"— local tokenizer registration undone"
            )
        return result
